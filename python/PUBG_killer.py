...
c = open('killer.csv')
killer = pd.read_csv(c)
killer = killer.loc[killer['deaths_description']!='DownandOut']
deaths = pd.DataFrame({})
for i in tqdm_notebook(range(1)):
    each_data = killer[['deaths_killer_position_x','deaths_killer_position_y','deaths_description','k-mean']]
...
for i in range(50):
    ...
    b = pd.DataFrame({'分群':[i],
                  '分群個數':[len(death)],
                    'x':[sum(death['deaths_killer_position_x'])/len(death)],
                    'y':[sum(death['deaths_killer_position_y'])/len(death)],
